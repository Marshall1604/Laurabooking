...
export function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#f3eee5] flex flex-col justify-between selection:bg-[var(--gold)] selection:text-[#0a0805]">
      {/* Inner Header */}
      <header className="sticky top-0 z-40 bg-[#050505]/95 backdrop-blur-md border-b border-[var(--border)] py-4">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {user?.role === 'admin' ? (
              <Link
                href="/admin"
                className="hidden sm:flex items-center gap-1.5 text-[0.68rem] tracking-[0.1em] uppercase text-[var(--gold-light)] bg-[var(--gold)]/10 border border-[var(--gold)]/30 px-3 py-1.5 rounded transition-colors"
              >
              ...
              <Link
                href="/account"
                className="hidden sm:flex items-center gap-1.5 text-[0.68rem] tracking-[0.1em] uppercase text-[#c5c0b8] hover:text-[var(--gold-light)] px-2.5 py-1.5 transition-colors"
              >
              ...
              <Link
                href="/auth/sign-in"
                className="hidden sm:flex items-center gap-1.5 text-[0.68rem] tracking-[0.1em] uppercase text-[#a5a098] hover:text-[var(--gold-light)] transition-colors px-2 py-1.5"
              >
                <User size={13} />
            ...
            <Link
              href="/booking"
              className="inline-flex items-center gap-1.5 px-3.5 py-2 text-[0.68rem] tracking-[0.14em] uppercase font-sans font-semibold text-[#0a0805] bg-gradient-to-r from-[#b79051] via-[#e3c98d] to-[#b68b4b] border border-[var(--gold-light)]"
            >
            ...
            <button
              ...
              className="lg:hidden p-1.5 text-white hover:text-[var(--gold)]"
            >
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-[#070707] border-b border-white/10 px-6 py-4 space-y-3 text-xs uppercase tracking-wider text-[#ccc] max-h-[80vh] overflow-y-auto font-sans">
            ...
            <Link href="/partner-with-us" onClick={() => setMobileMenuOpen(false)} className="block py-1 hover:text-[var(--gold-light)]">
              {t('nav_partner')}
            </Link>
            <Link href="/journal" onClick={() => setMobileMenuOpen(false)} className="block py-1 hover:text-[var(--gold-light)]">
              {t('nav_journal')}
            </Link>
            <Link href="/booking" onClick={() => setMobileMenuOpen(false)} className="block py-1 hover:text-[var(--gold-light)]">
              Booking
            </Link>
            <Link href="/account" onClick={() => setMobileMenuOpen(false)} className="block py-1 hover:text-[var(--gold-light)]">
              {t('nav_member_access')}
            </Link>
            {user?.role === 'admin' && (
              <Link href="/admin" onClick={() => setMobileMenuOpen(false)} className="block py-1 text-[var(--gold-light)] font-bold">
                {t('nav_admin')}
              </Link>
            )}
          </div>
      ...
      {/* Luxury Footer */}
      <footer className="border-t border-white/10 bg-[#070707] text-[#918c84] py-12">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-8 border-b border-white/5">
            <Link href="/" className="flex items-center gap-2 group">
              <Gem size={15} className="text-[var(--gold)]" />
              <span className="font-serif tracking-[0.22em] text-sm text-white font-medium">AURELIS</span>
            </Link>

            <div className="flex flex-wrap items-center justify-center gap-6 text-xs text-[#b0aba2] font-sans">
              <Link href="/services" className="hover:text-[var(--gold-light)]">{t('nav_services')}</Link>
              <Link href="/destinations" className="hover:text-[var(--gold-light)]">{t('nav_destinations')}</Link>
              <Link href="/partner-with-us" className="hover:text-[var(--gold-light)]">{t('nav_partner')}</Link>
              <Link href="/journal" className="hover:text-[var(--gold-light)]">{t('nav_journal')}</Link>
              <Link href="/account" className="hover:text-[var(--gold-light)]">{t('nav_member_access')}</Link>
              <Link href="/admin" className="hover:text-[var(--gold-light)]">{t('nav_admin')}</Link>
            </div>
          </div>

          <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-[0.68rem] text-[#6b665f] font-sans">
            <p>© 2026 AURELIS Private Experiences · All Rights Reserved.</p>
